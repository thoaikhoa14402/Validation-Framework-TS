...
function isIValidator<T>(value: any): value is IValidator<T> {
    ...
    return typeof(value as IValidator<T>).validate === "function";
}
...
class ObjectValidator<T extends object> implements IValidator<T> {
    ...
    validate(value: unknown, options = {stopOnFailure: true}, stack?: Stack<keyof T>) {
        ...
        if (this.objectSchema) {
            ...
            for (let expectedKey of expectedKeys) {
                ...
                const resultPerKey: Result<T> | ValidationError[] = isIValidator(validator) ? validator.validate(propVal, options, this.errorPath) : validator(propVal);
